...
function CalorieForm({ setTotalCals, setRecommendation }) {
	...
	const calorieEquation = () => {
		...
		totalCalories > 0
			? setTotalCals(Math.round(totalCalories))
			: setTotalCal(0);
	};
		...
		<div className='form_wrapper'>
			<form onSubmit={onSubmit} className='calorie_form'>
				...
				<div className='row_container'>
					...
					<input
						...
						onChange={(e) =>
							e.target.value < 0
								? setFeet(0)
								: setFeet(Number(e.target.value) * 12)
						}
					></input>
					<input
						type='number'
				...
				<div className='btn_wrapper'>
					<button
						className='calorie_btn'
						type='button'
						onClick={calorieEquation}
					>
						Calculate
					</button>
				</div>
				...
				<div className='row_container'>
					...
					<input
						type='number'
						min='0'
						onChange={(e) => setTotalCals(Number(e.target.value))}
					></input>
				</div>
				<div className='btn_wrapper'>
					<button type='submit' className='calorie_btn'>
						Submit
					</button>
				</div>
			</form>
